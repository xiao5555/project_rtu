...
static	OS_MSG         OSCfg_MsgPool[OS_CFG_MSG_POOL_SIZE];
static	uint16_t      const   OSCfg_MsgPoolSize           = OS_CFG_MSG_POOL_SIZE;
static	uint32_t      const  OSCfg_MsgPoolSizeRAM        = sizeof(OSCfg_MsgPool);
static	OS_MSG       * const  OSCfg_MsgPoolBasePtr       = (OS_MSG    *)&OSCfg_MsgPool[0];
static  OS_MSG_POOL               OSMsgPool;                  /* Pool of OS_MSG                             */
/*
************************************************************************************************************************
                  ...
                  uint8_t       *p_err)
{
    ...
    if (p_msg_q->NbrEntries >= p_msg_q->NbrEntriesSize) {
       *p_err = 2;                               /* Message queue cannot accept any more messages          */
        return;
    }

    if (OSMsgPool.NbrFree == (OS_MSG_QTY)0) {
       *p_err = 3;                      /* No more OS_MSG to use                                  */
        return;
    }
